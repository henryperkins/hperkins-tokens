...
node scripts/verify-job-placement-digest-source.js # recruiter main/appendix copy, outline, evidence, row counts, fragment target, immutable links, and retired-pattern contract
...
node scripts/apply-local-page-drafts.js --confirm-local # guarded localhost-only application of the two reviewed recruiter drafts (MUTATES local pages)
