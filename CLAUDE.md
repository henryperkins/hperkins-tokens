...
- **`templates/` + `parts/`** — front-page (the Three Rings page), home/single (blog index + reader), page-about, page-ai-enablement, page-case-study, page-contact, page-work, page-plato-artifacts; header/footer parts. Unspecified templates are **inherited from Assembler**. Note `page-ai-enablement.html` is a **shadow template** over a live page (DB content untouched), and `page-plato-artifacts.html` preserves that page's DB content inside the work-template shell. At `/`, `front-page.html` intentionally outranks `home.html`.
