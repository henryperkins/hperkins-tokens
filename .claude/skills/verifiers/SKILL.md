...
node scripts/verify-homepage-hero-polish.js     # / at 320/390/600/601/781/900/901/1024/1440 plus no-JS at the primary boundaries: semantic + rendered eyebrow → H1 → Wapuu → support order, 900px grid transition without CSS order, touch targets, containment, section geometry, and medium-mobile/semibold-desktop title weight
...
node --test scripts/lib/search-policy.test.js    # PHP search filters: AI disabled, token highlights, composed utility-page exclusions, native search parity, localized recovery
node --test scripts/lib/council-header-block.test.js # native block conversion and unsaved editor preview-state checks
wp eval-file scripts/verify-council-header-block.php # local read-only WordPress integration: registration, shared editor CSS, public/legacy parity, preview isolation and label/ID integrity (set --path explicitly)
node scripts/verify-journal-polish.js           # /essays/ masthead clamp + overflow at 390/320px; fallback plate-crop variety
node scripts/verify-about-page-rendered.js      # /about/ phase-aware rendered contract at 1440/1024/768/390/320 plus boundary probes: derives the proof-first accepted-snapshot, v2 résumé, or v3 résumé shape from the selected body, then checks heading ancestry, named navigation + keyboard fragment behavior, responsive work/showcase layout, portrait text, prominent actions, focus, overflow, reduced motion, source/rendered word-count parity, and screenshots. --source-only --drafts checks the v3 candidate; --require-local --drafts runs it against the matching local install (needs HPERKINS_WP_PATH + non-production HPERKINS_ORIGIN)
...
node scripts/verify-deployed-content-ownership.js # public post-deploy gate: About + Digest + Method DB bodies must equal committed snapshots (--drafts compares a reviewed local/live candidate; --source-only checks route wiring)
...
node scripts/verify-ai-enablement-source.js     # /ai-enablement/ adapter contract: executes patterns/ai-enablement.php under a minimal WordPress shim (needs `php` on PATH or HPERKINS_PHP_BIN) and requires byte parity with content/page-snapshots/ai-enablement.html on the accepted body with only the six Three Rings image URLs re-derived, the same parity for a round-tripped body (another host, no ?v=), and empty output for a missing, empty, unmatched, or duplicated snapshot or one whose asset survives only in a non-native attribute such as data-src; also pins the source policy (reads the snapshot, no draft, no essay markup) and the snapshot's side (each asset exactly once as a native src/srcset, no other theme-hosted URL)
